...
const RslPagination: FC<PaginationProps> = (props) => {
    ...
    function getNextDisplayPageCount(displayPageBtn: number, totalItem: number) {
        const totalPage = Math.round(totalItem / state.perPageRow)
        return totalPage >= displayPageBtn ? displayPageBtn : totalPage
    }

    function calcPaginationView(totalItem: number, _perPageRow: number) {
        const arr: number[] = []
        const totalPage = Math.round(totalItem / state.perPageRow)
